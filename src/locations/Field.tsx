	...
	const renderCustomChildren = useCallback(
		(props: WrappedEntryCardProps) => {
			...
			const result = fields.map((field) => {
				const { schema, value } = getFieldFromEntry({
					field,
					contentType,
					entry,
				...
				if (!schema) {
					return (
						<Note key={field} variant='warning'>
							...
							<Text>
								Remove it from the definition of <em>{sdk.field.name}</em> or
								add it to the content model of <em>{contentType?.name}</em>.
								<br />
							</Text>
						</Note>
					);
				...
				return <FieldDisplay key={field} label={schema?.name} value={_value} />;
			});
		...
		[sdk.field.locale, sdk.field.name, fields]
	);
